...
namespace Aplication.IntegradorCRM.Servicos.Boleto
{
    internal class CriacaoService
    {
        internal async static Task<bool> RealizarProcessoCriacaoBoleto(DadosAPIModels DadosAPI, RetornoBoleto boleto, bool EnviarPDFaoCriarOPT, bool enviarPDFPorEmailAoCriarOPT, ConfigEmail configEmail)
        {
            ...
            ModeloOportunidadeRequest OportunidadRequest = await Boleto_Services.InstanciarAcaoRequestSitucaoBoleto(boleto, Situacao_Boleto.AbertoOuABertura);

            EmailModel Email = await Boleto_Services.InstanciarAcaoRequesEmailtSitucaoBoleto(boleto, Situacao_Boleto.AbertoOuABertura);

            Email.destinatario = RelacaoBoleto.Email_Entidade;
            Email.assunto = "Boleto";
            
           
            // Tenta enviar a mensagem de criacão de boleto, caso nao der certo a criacão, gera um log
            if (await EnviarMensagemCriacao(OportunidadRequest, DadosAPI, _dalRelacaoBOletos, RelacaoBoleto, EnviarPDFaoCriarOPT, enviarPDFPorEmailAoCriarOPT, configEmail, Email) is false)
            {
        ...
        private async static Task<bool> EnviarMensagemCriacao(ModeloOportunidadeRequest Request, DadosAPIModels DadosAPI, DAL<RelacaoBoletoModel> dalRelBoletos, RelacaoBoletoModel RelacaoBoleto, bool enviarPDFaoCriarOPT, bool enviarPDFPorEmailAoCriarOPT,  ConfigEmail configEmail, EmailModel emailModel)
        {
            ...
            bool response = await EnviarMensagemBoleto.EnviarMensagemCriacao(Request, DadosAPI, dalRelBoletos, RelacaoBoleto, enviarPDFaoCriarOPT, configEmail, enviarPDFPorEmailAoCriarOPT, emailModel);
            return response;
        }
    } 
}
