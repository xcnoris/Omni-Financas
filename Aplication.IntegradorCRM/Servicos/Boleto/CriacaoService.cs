...
namespace Aplication.IntegradorCRM.Servicos.Boleto
{
    internal class CriacaoService
    {
        internal async static Task<bool> RealizarProcessoCriacaoBoleto(DadosAPIModels DadosAPI, RetornoBoleto boleto, bool EnviarPDFaoCriarOPT, bool enviarPDFPorEmailAoCriarOPT, ConfigEmail configEmail)
        {
            ...
            if (await EnviarMensagemCriacao(OportunidadRequest, DadosAPI, _dalRelacaoBOletos, RelacaoBoleto, EnviarPDFaoCriarOPT, enviarPDFPorEmailAoCriarOPT, configEmail) is false)
            {
        ...
        private async static Task<bool> EnviarMensagemCriacao(ModeloOportunidadeRequest Request, DadosAPIModels DadosAPI, DAL<RelacaoBoletoModel> dalRelBoletos, RelacaoBoletoModel RelacaoBoleto, bool enviarPDFaoCriarOPT, bool enviarPDFPorEmailAoCriarOPT,  ConfigEmail configEmail)
        {
            ...
            bool response = await EnviarMensagemBoleto.EnviarMensagemCriacao(Request, DadosAPI, dalRelBoletos, RelacaoBoleto, enviarPDFaoCriarOPT, configEmail, enviarPDFPorEmailAoCriarOPT);
            return response;
        }
    } 
}
